...
function CreditRunner() {
    ...
    this.openCreditPage = function(tryTime) {
        ...
        sleep(500)
        if (widgetUtils.widgetWaiting('红包最高100元|王者荣耀红包封面|抢兑第五人格周边')) {
            let myWidget = widgetUtils.widgetGetOne('红包最高100元|王者荣耀红包封面|抢兑第五人格周边')
            automator.clickCenter(myWidget)
        ...
        if (widgetUtils.widgetWaiting('领取按钮')) {
            ...
            automator.clickCenter(myWidget)
            FloatyInstance.setFloatyText('领取按钮')
        }
        FloatyInstance.setFloatyText('校验是否有打开确认弹框')
            // confirm = widgetUtils.widgetGetOne(/^打开$/, 3000)
            // if (confirm) {
            //     this.displayButtonAndClick(confirm, '找到了打开按钮')
            // } else {
            //     FloatyInstance.setFloatyText('没有打开确认弹框')
    ...
    this.doTask = function(limit) {
            ...
            if (widgetUtils.widgetWaiting('去看看|去完成|去逛逛')) {
                ...
                FloatyInstance.setFloatyText('跳转到任务界面')
            } else {
            ...
            this.pushLog('等待页面完全刷新 等待2秒')
            let toFinishList;
            let more = widgetUtils.widgetGetOne('查看更多')
            if (more) {
                automator.clickCenter(more)
            }
            if (widgetUtils.widgetWaiting('去看看|去完成|去逛逛')) {
                // let myWidget = widgetUtils.widgetGetOne('休闲时间玩一把|小游戏推荐|限时1000连抽|一键打开|超爽末日|小游戏')
            ...
            // if (widgetUtils.widgetWaiting('休闲时间玩一把|小游戏推荐|限时1000连抽必中SSR|一键打开|超爽末日|巨上头的休闲小游戏|不烧脑|无门槛支付立减')) {
            //     let myWidget = widgetUtils.widgetGetOne('休闲时间玩一把|小游戏推荐|限时1000连抽|一键打开|超爽末日|小游戏')

            //     FloatyInstance.setFloatyText('跳转到任务界面')
            // }
            toFinishList = widgetUtils.widgetGetAll('去看看|去完成|去逛逛')
            if (!toFinishList || toFinishList.length <= 0) {
                this.pushLog('无可完成任务')
                signFailedUtil.recordFailedWidgets(this.taskCode, '每日任务')
                let changeTask = widgetUtils.widgetGetOne('查看更多', 1000)
                if (changeTask) {
            ...
            if (task && task.executable) {
                ...
                sleep(1000)
                if (!widgetUtils.widgetWaiting('赚更多青春豆', null, 2000)) {
    ...
    function findFirstExecutableTask(buttonList) {
        ...
        let taskBuilderList = [
            ...
            () => buildTask((btnInfo) => {
                ...
                function(btn) {
                    this.pushLog('小程序游戏任务 点击进入 等待20秒 然后返回')
                    let limit = 20
                    while (limit-- > 0) {
                        this.replaceLastLog('小程序任务等待' + limit + '秒')
                        sleep(1000)
                    }
                    automator.click(1091 + 36, 159 + 36)
                    sleep(1000)
                }
            ),
            ...
            if (resultList.length > 0) {
                return {
                    ...
                    execute: function() {
                        ...
                        let btn = this.ensureTargetInVisible(target.btn, [0, config.device_height * 0.1, config.device_width, config.device_height * 0.8],
    ...
    this.doBrowseTask = function() {
        ...
        let browser15 = widgetUtils.widgetGetAll('兑换区tab图片', 3000)
        if (browser15 && browser15.length > 1) {
            let sweap = 3
            while (browser15[1].bounds().centerY() > config.device_height && sweap > 0) {
                sweap--;
                automator.gestureDown(startY, endY)
                browser15 = widgetUtils.widgetGetAll('兑换区tab图片', 3000)
            }
            automator.clickCenter(browser15[1])
