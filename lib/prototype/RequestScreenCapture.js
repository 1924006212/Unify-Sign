...
module.exports = function (landscape) {
  ...
  let confirmWaitingThread = threads.start(function () {
    ...
    let confirmWidget = widgetUtils.widgetGetOne(config.capture_permission_button || 'START NOW|立即开始|允许',2000)
    if (confirmWidget) {
      sleep(200)
	  ...
	  let confirmWidget = widgetUtils.widgetGetOne('Cancel|取消|')

	  if(confirmWidget){
		  let x = confirmWidget.bounds().centerX()+confirmWidget.bounds().width, y =confirmWidget.bounds().centerY()
		  automator.click(x,y)
	  }else{
